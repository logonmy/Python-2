...
class SpiderMain(object):
                ...
                if len(items) > 0:
                    for index in range(0, len(items)):
                        ...
                        if source_title2 is not None:
                            _rs = source_title2.find("span")
                            _rsv = _rs.find("value") if _rs is not None else None
                            _reference_source = _rsv.text if _rsv is not None else ''
        ...
        if response.status_code == 200:
            ...
            if search_results is not None:
                ...
                if search_results_item_list is not None and len(search_results_item_list) > 0:
                    result = []
                    for item in search_results_item_list:
                        search_results_content = item.find("div", class_="search-results-content")
                        if search_results_content is not None:
                            a_href = search_results_content.find("a")["href"]
                            if a_href is not None:
                                result.append(self.base_url + a_href)
                    return result
        return None
    ...
    def loop_page_byTotalNum(self, url):
        ...
        while current_page <= total_page:
            print(self.get_nowTime() + "本条论文的参考文献第%s页开始爬取，爬取%d条数据" % (current_page, page_size))
            # 循环抓取数据
            ...
            print(self.get_nowTime() + "本条论文的参考文献爬取第%s页数据成功！" % current_page)
            print(self.get_nowTime() + "--------------------忧伤的分割线----------------------")
            current_page += 1
        response.close()
        return result_list
    ...
    def craw_detail(self, url, it):
        ...
        if response.status_code == 200:
            ...
            if datacontainer is not None:
                    ...
                    for item in block_record_info_item:
                        ...
                        if title3_option == None:
                                ...
                                if brisv is not None:
                                    ...
                                    if bvs is not None:
                                        for bv in bvs:
                                            ...
                                            elif bv.find("span", class_="FR_label").text == "子辑:" or bv.find("span", class_="FR_label").text == "Part:":
                                                part = bv.find("value").text
                            ...
                            # 基金资助致谢
                                ...
                                # funding_text
                                funding_text = item.find(id="show_fund_blurb").find("p").text if item.find(id="show_fund_blurb") is not None else ''
                            # 出版商
                ...
                sql = 'REPLACE INTO wos_thesis_publication(id,title,accession_number,source,issn,eissn,publisher,publish_year,volume,issue,page_begin,page_end,part,ids_number,isbn,book_series,special_issue) ' \
                      'VALUES ("' + id + '","' + title + '","' + accession_number + '","' + source + '","' + issn + '","' + eissn + '","' + publisher + '","' + publish_year + '","' + volume + '","' + issue + '","' + page_begin + '","' + page_end + '","' + part + '","' + ids_number + '","' + isbn + '","' + book_series + '","' + special_issue + '") '
                sqls.append(sql)
    ...
    def go_to_craw(self, title, author, it):
        ...
        # print("sId=", sId)
        # print("获取sId结束、、、、、、、、、、、")
        urls = self.crawl_url(sId, title, author)
        if urls is not None and len(urls) > 0:
            for url in urls:
                self.craw_detail(url, it)
        else:
            print(self.get_nowTime() + "查询获取prId失败,已经发送至redis数据库")
            self.redis_client.lpush(self.consumer_list_success_fail, json.dumps(it))

    # 程序入口
    def craw(self):
        print(self.get_nowTime() + "爬虫开始爬取数据、、、")
                ...
                for item in items:
                    ...
                    if exist is False:
                        if len(list_authors) > 0:
                            try:
                                print(self.get_nowTime() + "kafka----topic:%s,partition:%s,offset:%s 消费开始" % (item.topic, item.partition, item.offset))
                                print(str(self.get_nowTime()) + str(it))

                                self.go_to_craw(title=thesis_title, author=list_authors[0], it=it)
                                self.redis_client.lpush(self.consumer_list_success, json.dumps(it))
                                print(self.get_nowTime() + "kafka----topic:%s,partition:%s,offset:%s 消费成功！" % (item.topic, item.partition, item.offset))
                                ...
                                print(self.get_nowTime() + "消费失败，已写入redis消费失败列表")
                    else:
                        print(self.get_nowTime() + "已经存在，无需爬取:%s" % (thesis_title))
